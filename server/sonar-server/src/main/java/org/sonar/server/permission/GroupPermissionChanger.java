...
public class GroupPermissionChanger {
    ...
    switch (change.getOperation()) {
      case ADD:
        ...
        GroupPermissionDto addedDto = new GroupPermissionDto()
          ...
          .setResourceId(change.getProjectId());
  ...
  private boolean shouldSkip(DbSession dbSession, GroupPermissionChange change) {
    ...
    if (change.getGroupIdOrAnyone().isAnyone()) {
      existingPermissions = dbClient.groupPermissionDao().selectAnyonePermissions(dbSession, change.getProjectId());
    } else {
      existingPermissions = dbClient.groupPermissionDao().selectGroupPermissions(dbSession, change.getGroupIdOrAnyone().getId(), change.getProjectId());
    }
    return (PermissionChange.Operation.ADD == change.getOperation() && existingPermissions.contains(change.getPermission())) ||
      (PermissionChange.Operation.REMOVE == change.getOperation() && !existingPermissions.contains(change.getPermission()));
  }
